...
from src.training_utils.util.data_utils import l1_dist
from src.training_utils.util.graph_utils import graph_to_tensor
from src.training_utils.util.image_id_dict import get_image_numbers
from src.training_utils.util.mean_std import mean, std
from src.training_utils.util.semantics_dict import semantics_dict


class MyDataset(Dataset):
    def __init__(self, dataset_path, img_path, annot_path, extract_roi):
    ...
    def __getitem__(self, index):
        ...
        if 1:
            ...
            orig_quadtree = np.load(os.path.join(self.quadtree_path, img_file_name[:-4] + '.npy'), allow_pickle=True).item()['quatree'][0]
            quadtree = {}
            ...
            image_id = torch.tensor([get_image_numbers(data_path=self.dataset_path)[img_id]])
