...
import src.training_utils.util.misc as utils
from args import get_args_parser
from src.training_utils.datasets.dataset import MyDataset
from engine import train_one_epoch, evaluate_iter
from src.training_utils.models.build import build_model, build_criterion, build_postprocessor
from src.training_utils.util.output_utils import make_outputdir_and_log
from src.training_utils.util.param_print_utils import match_name_keywords
from src.training_utils.util.random_utils import set_random_seed
import numpy as np
...
dataset_train = MyDataset(args.dataset_path, args.dataset_path + '/train', args.dataset_path + '/annot_json' + '/instances_train.json', extract_roi=False)
dataset_val = MyDataset(args.dataset_path, args.dataset_path + '/valid', args.dataset_path + '/annot_json' + '/instances_valid.json', extract_roi=False)
...
for epoch in range(start_epoch, max_epoch):
    # train 1 epoch
    train_one_epoch(task, model, criterion, data_loader_train, optimizer, epoch, args.clip_max_norm, args)
    lr_scheduler.step()
    # save a checkpoint
    if args.output_dir:
